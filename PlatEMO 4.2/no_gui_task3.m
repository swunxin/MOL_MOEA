% Headless BindingDB P3 runner.
% P3 objective is implemented in optimizer_task13.py:
%   OBJECTIVE_MODE=bindingdb_p3
%   QED + DRD2 + SA_norm + Similarity
%   success: QED >= 0.80, DRD2 >= 0.40, SA_norm >= 0.70, sim >= 0.30
%
...
fprintf('[MATLAB-P3] source_dir=%s\n', platemo_source_dir);
fprintf('[MATLAB-P3] comm_dir=%s\n', matlab_location);

removeAllFiles(matlab_location);
disp('[MATLAB-P3] Deleted communication text files to start a fresh run.');
py_shutdown_file = fullfile(matlab_location, 'py_SHUTDOWN.txt');

if ~exist('lead_start', 'var') || ~exist('lead_end', 'var')
    disp('[MATLAB-P3] Using default lead_range: 1 to all available leads');
    lead_start = 1;
    lead_end = [];
else
    fprintf('[MATLAB-P3] Using assigned lead_range: %d to %d\n', lead_start, lead_end);
end
...
env_auto_retry = firstNonEmptyEnv({'TASK3_AUTO_RETRY', 'TASK1_AUTO_RETRY'});
auto_retry = any(strcmpi(strtrim(env_auto_retry), {'1','true','yes','on'}));
env_max_attempts = firstNonEmptyEnv({'TASK3_MAX_ATTEMPTS', 'TASK1_MAX_ATTEMPTS'});
...
tre_q = readEnvDouble('TASK3_SUCCESS_QED', 0.80);
tre_d = readEnvDouble('TASK3_SUCCESS_DRD2', 0.40);
tre_a = readEnvDouble('TASK3_SUCCESS_SA_NORM', 0.70);
tre_s = readEnvDouble('TASK3_SUCCESS_SIM', 0.30);

fprintf('[MATLAB-P3] algorithm=%s base_seed=%d maxFE=%d autoRetry=%d maxAttempts=%d thresholds=(QED>=%.3f,DRD2>=%.3f,SA_norm>=%.3f,SIM>=%.3f)\n', ...
    alg_name, base_seed, max_fe, auto_retry, max_attempts, tre_q, tre_d, tre_a, tre_s);

start_run(alg_handle, alg_name, lead_start, lead_end, base_seed, max_fe, auto_retry, max_attempts, tre_q, tre_d, tre_a, tre_s, matlab_location, platemo_source_dir);

writematrix([], py_shutdown_file);
fprintf('[MATLAB-P3] Wrote shutdown file: %s\n', py_shutdown_file);
...
function start_run(alg, alg_name, lead_start, lead_end, base_seed, max_fe, auto_retry, max_attempts, tre_q, tre_d, tre_a, tre_s, matlab_location, source_dir)
    ...
    lead_smiles = loadLeadSmiles(source_dir, 'TASK3_LEAD_FILE', 'TASK3_DATA_DIR', 'p3_lead_smiles.txt', 'p3_drd2_qsa_lead_smiles.txt');
    lead_smiles = cleanLeadLines(lead_smiles);
    if isempty(lead_smiles)
        error('P3 lead source is empty.');
    end

    run_range = getRunRange('TASK3_MOL_IDS', lead_start, lead_end, numel(lead_smiles));
    fprintf('[MATLAB-P3] Loaded %d leads (will run %s)\n', numel(lead_smiles), rangeToText(run_range));
    ...
    for i = run_range
        ...
        for attempt = 1:max_attempts
            ...
            fprintf('[MATLAB-P3-RUN] lead_idx=%d lead_id=%d attempt=%d/%d seed=%d\n', lead_idx, lead_id0, attempt, max_attempts, seed_i);
            ...
            before_result_files = snapshot_result_files(source_dir, alg_name, 4);
            platemo('algorithm', alg, 'problem', @DDProblem1, 'maxFE', max_fe, 'save', 100000);

            result_file = find_new_result_file(source_dir, alg_name, 4, before_result_files);
            [success_final, success_any, best_q, best_d, best_a, best_s] = p3_success_from_result(result_file, tre_q, tre_d, tre_a, tre_s);
            append_retry_summary(matlab_location, lead_id0, lead_idx, attempt, seed_i, success_final, success_any, best_q, best_d, best_a, best_s, result_file);
            fprintf('[MATLAB-P3-RETRY] lead_id=%d attempt=%d/%d successFinal=%d successAny=%d bestQED=%.4f bestDRD2=%.4f bestSA_norm=%.4f bestSim=%.4f\n', ...
                lead_id0, attempt, max_attempts, success_final, success_any, best_q, best_d, best_a, best_s);
            ...
            if auto_retry && attempt < max_attempts
                fprintf('[MATLAB-P3-RETRY] lead_id=%d failed attempt %d; retrying with a new seed.\n', lead_id0, attempt);
            end
        end

        if auto_retry && ~lead_success
            fprintf('[MATLAB-P3-RETRY] lead_id=%d failed after %d attempts.\n', lead_id0, max_attempts);
        end
    end
end
...
function lead_smiles = loadLeadSmiles(source_dir, file_env, dir_env, default_name, fallback_name)
    ...
    for c = 1:numel(candidate_files)
        ...
        if ~isempty(cand) && isfile(cand)
            ...
            fprintf('[MATLAB-P3] Lead source: %s\n', cand);
            return;
        end
    end
    error('Missing P3 lead source. Set %s or provide data/leads/%s.', file_env, default_name);
end
...
function [success_final, success_any, best_q, best_d, best_a, best_s] = p3_success_from_result(result_file, tre_q, tre_d, tre_a, tre_s)
    ...
    for r = 1:size(S.result, 1)
        ...
        if any(valid)
            success_any = success_any | any((q(valid) >= tre_q) & (d(valid) >= tre_d) & (sa(valid) >= tre_a) & (sim(valid) >= tre_s));
        end
        final_q = q;
        final_d = d;
        final_a = sa;
    ...
    success_final = any((final_q(final_valid) >= tre_q) & (final_d(final_valid) >= tre_d) & (final_a(final_valid) >= tre_a) & (final_s(final_valid) >= tre_s));
    best_q = max(final_q(final_valid));
    best_d = max(final_d(final_valid));
    best_a = max(final_a(final_valid));
    best_s = max(final_s(final_valid));
end

function append_retry_summary(matlab_location, lead_id0, lead_idx, attempt, seed_i, success_final, success_any, best_q, best_d, best_a, best_s, result_file)
    ...
    if write_header
        fprintf(fid, 'mol_id,lead_idx,attempt,seed,success_final,success_any,best_qed,best_drd2,best_sa_norm,best_sim,result_file\n');
    end
    fprintf(fid, '%d,%d,%d,%d,%d,%d,%.6f,%.6f,%.6f,%.6f,"%s"\n', ...
        lead_id0, lead_idx, attempt, seed_i, success_final, success_any, best_q, best_d, best_a, best_s, result_file);
    clear cleaner;
end
