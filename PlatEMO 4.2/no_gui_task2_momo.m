% MOMO Task2 (pLogP + Similarity) runner.
% Run this with the Python optimizer using OBJECTIVE_MODE=momo_task2.
%
% This file mirrors no_gui_task1_momo.m:
%   - per-worker communication directory through comm_dir_override
%   - 0-based mol_id written for MOMO alignment
...
disp("Deleted communication text files to start a fresh MOMO Task2 run.");
py_shutdown_file = fullfile(matlab_location, 'py_SHUTDOWN.txt');

if ~exist('lead_start', 'var') || ~exist('lead_end', 'var')
    disp('Using default MOMO Task2 lead_range: 1 to all available leads');
    lead_start = 1;
    lead_end = [];
else
    disp(['Using parallel assigned MOMO Task2 lead_range: ', num2str(lead_start), ' to ', num2str(lead_end)]);
end
...
function start_run(alg, lead_start, lead_end, base_seed, matlab_location, source_dir)
    ...
    lead_smiles = loadTask2LeadSmiles(source_dir);
    if isempty(lead_smiles)
        error('MOMO Task2 lead source is empty.');
    end
    ...
    fprintf('Loaded %d MOMO Task2 leads (will run %d to %d)\n', numel(lead_smiles), lead_start, lead_end);
    ...
    for i = run_range
        ...
        if i > numel(lead_smiles)
            error('Requested lead index %d exceeds available Task2 leads (%d)', i, numel(lead_smiles));
        end
        ...
        fprintf('[MATLAB-MOMO-TASK2] lead_idx=%d lead_id=%d seed=%d smiles=%s\n', lead_idx, lead_id0, seed_i, smi);
...
function lead_smiles = loadTask2LeadSmiles(source_dir)
    candidate_files = {};
    env_lead_file = getenv('TASK2_LEAD_FILE');
    env_data_dir = getenv('TASK2_DATA_DIR');
    if ~isempty(env_lead_file)
        candidate_files{end+1} = env_lead_file;
    end
    if ~isempty(env_data_dir)
        candidate_files{end+1} = fullfile(env_data_dir, 'logp_test.csv');
        candidate_files{end+1} = fullfile(env_data_dir, 'plogp_testval.csv');
        candidate_files{end+1} = fullfile(env_data_dir, 'lead_smiles.txt');
    end
    candidate_files{end+1} = fullfile(source_dir, '..', '..', 'MOMO-master-main', 'momo', 'data', 'logp_test.csv');
    candidate_files{end+1} = fullfile(source_dir, '..', '..', 'MOMO-master-main', 'momo', 'data', 'plogp_testval.csv');
    ...
    for c = 1:numel(candidate_files)
        ...
        if ~isempty(cand) && isfile(cand)
            ...
            lead_smiles = strings(numel(lines), 1);
            for k = 1:numel(lines)
                ...
                if contains(line, ',')
                    ...
                    parts = split(line);
                    lead_smiles(k) = parts(1);
                end
            end
            fprintf('MOMO Task2 lead source: %s\n', cand);
            return;
        end
    end

    error('Missing MOMO Task2 lead source. Set TASK2_LEAD_FILE or provide MOMO-master-main/momo/data/logp_test.csv.');
end
