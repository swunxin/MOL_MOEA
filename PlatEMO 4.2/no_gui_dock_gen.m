...
% This de novo dock_gen script relies on Python to write py_M.txt (M=2: QED + Docking).
...
% Lead generation pilot: start with a single random seed
random_states = [42];
...
start_run(@MOEADD, 1:1, random_states, matlab_location);
...
function start_run(alg, run_range, random_states, matlab_location)
    ...
    for i = run_range
        ...
        % Smaller FE budget for de novo lead generation to avoid over-optimizing
        % before downstream soft filtering / lead-init selection.
