classdef MOL_MOEA_v15_nobank < ALGORITHM
% MOL_MOEA_v15_nobank: == MOL_MOEA_v12_envSPEA2, 唯一改动 = **强制 bankOn=0**
%   (ParameterSet 之后硬覆盖)。-> L0/L1/L2 全关, BANK 既不建也不用, OutputPop=Population。
%   = 纯 SPEA2 环境选择 + RL 控制器, 无任何 archive。
%
%   目的(消融): 测"L1 输出合并/BANK 对分子到底有没有必要"。对比 v12(带 L1, 中位19):
%     若分子中位/HV ≈ v12 -> BANK 可去, 论文连存档都不提, 最干净的 learning-based EC;
%     若明显低于 v12 -> L1 存档对分子多样性有真贡献, 保留为分子侧组件。
%   LSMOP 上 v12 本就 bank inert(objThr=[] 门控关), 故本版 LSMOP == v12, 仅分子有区别。
%
%   [以下为 v12 原说明]
...
%   temperature stays fixed at 0.7. reward/pool/SOM unchanged.
    methods
        function main(Algorithm,Problem)
            ...
            bankOn = 0;   % v15_nobank: 硬覆盖 -> 关掉 L0/L1/L2, 纯 SPEA2 无 archive (无视 driver 传的 bankOn)
