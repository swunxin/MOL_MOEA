classdef MOL_MOEA_v11_pooltrace2 < ALGORITHM
% MOL_MOEA_v11_pooltrace2: == MOL_MOEA_v11_pooltrace, 仅在每点统计里多加 pass=K
%   (满足 objThr 双阈值=Task1 达标 qed>=0.9 & sim>=0.4 的点数)。搜索逻辑一字不改。
%   用途(分析 a): trace 一个成功 lead, 看 pass 从哪代>0(成败分水岭) + 前沿是否解冻,
%   对比失败 lead5(全程 pass=0, 前沿 g23 起冻死 227 代)。
%   每代打印三个观测点: OFF=算子产出 Offspring, SEL=EnvironmentalSelection 后(Pop_Pool 前),
%   POOL=Pop_Pool 后。每点报 N / 唯一目标点 uObj / 唯一潜向量 uDec / pass / 目标范围。
...
%   temperature stays fixed at 0.7. reward/pool/SOM unchanged.
    methods
        function main(Algorithm,Problem)
            ...
            genCounter = 0;   % [pooltrace] 仅诊断打印,不改搜索
            while Algorithm.NotTerminated(OutputPop)
                ...
                pop_sel = Population;   % [pooltrace] 选择后、Pop_Pool 前
                Population = Pop_Pool(Population, Population_temp, Offspring, Problem);
                genCounter = genCounter + 1;   % [pooltrace] 打印三个观测点的多样性统计
                fprintf('[PT] g=%d FE=%d | OFF %s | SEL %s | POOL %s\n', ...
                    genCounter, Problem.FE, pt_stat(Offspring,objThr), pt_stat(pop_sel,objThr), pt_stat(Population,objThr));
...
function s = pt_stat(P, objThr)
% [pooltrace2] 多样性统计: N / 唯一目标点 / 唯一潜向量(结构近似) / pass / obj1,obj2 范围
%   pass = 满足 objThr 双阈值的点数(Task1 达标 qed>=0.9 & sim>=0.4)。objThr=[] -> pass=NA。
    if nargin < 2; objThr = []; end
    if isempty(P)
        s = 'N=0'; return;
    end
    O = P.objs; Dd = P.decs;
    nuO = size(unique(round(O,6),'rows'),1);          % 不同目标点
    nuD = size(unique(round(Dd,4),'rows'),1);         % 不同潜向量
    if ~isempty(objThr)
        nPass = sum(all(O <= objThr, 2));             % 达标点数(同 BANK/输出的达标判据)
        passStr = sprintf('pass=%d', nPass);
    else
        passStr = 'pass=NA';
    end
    if size(O,2) >= 2
        s = sprintf('N=%d uObj=%d uDec=%d %s o1[%.3f,%.3f] o2[%.3f,%.3f]', ...
            numel(P), nuO, nuD, passStr, min(O(:,1)),max(O(:,1)),min(O(:,2)),max(O(:,2)));
    else
        s = sprintf('N=%d uObj=%d uDec=%d %s', numel(P), nuO, nuD, passStr);
    end
end