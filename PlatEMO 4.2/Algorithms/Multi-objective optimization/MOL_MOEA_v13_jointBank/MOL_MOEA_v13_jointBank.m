classdef MOL_MOEA_v13_jointBank < ALGORITHM
% MOL_MOEA_v13_jointBank: == MOL_MOEA_v12_envSPEA2 的进化(SPEA2环境选择)一字不动,
%   唯一改动 = 分子任务下 BANK 家族(updateBank/mergeForOutput)的多样性度量空间。
%
%   动机(2026-06-16 BANK败笔审计): v12 的 BANK 用"归一化潜向量距离"做 niche 准入/迁移/
%   淘汰, 而 潜向量 != 分子(VAE 多对一) -> 潜远=同分子留成冗余、潜近=不同分子被并/覆盖丢结构。
%   且 mergeForOutput 是 diversity-blind 贪心(支配/替worst-by-sum), BANK 多样性到不了被测种群。
%   拆 FRCSO 的唯一达标结构数 = (a)达标点在目标空间铺开(sim跨度~0.09,多个目标点) +
%   (b)同一目标点上挂多个不同结构。纯目标密度截断治(a)杀(b)(同目标不同结构被判最拥挤删掉);
%   纯潜距离治(b)但被多对一假多样性污染。
%
%   v13 解法 = 联合(目标⊕潜)空间密度: 只有"目标 AND 潜 同时都近"才算冗余(同目标但潜远 =
%   不同结构 -> 留)。两块归一化后按维度数做均值平衡(避免256维潜淹没2维目标)。又因输出槽位(~N=100)
%   远多于所需达标点(~十几), 默认不激进截断 —— mergeForOutput 把"所有不同的达标点尽量送进输出",
%   仅当超容量才按联合密度删最冗余者(同目标同潜=几乎同分子)。这同时吃 (a)(b), 是真双空间。
%
%   分子级SMILES去重(真结构信号)留作方向B(MATLAB无解码器), 这版用 目标⊕潜 作最佳可得代理。
%
%   --- 任务门控(铁律) ---
%   objThr=[] (LSMOP, task=0): updateBank 走 v12 niche 原码一字不动, mergeForOutput 不被调用
%   (调用处已 gated ~isempty(objThr)) -> LSMOP == v12, 刚赢的 rank 3.33 gate 不受影响。
%   注入(L2)仍默认关, 且加 objThr 守卫 -> 将来只对分子开, LSMOP 永久关。
%
%   [以下为 v12 原说明]
    ...
    methods
        function main(Algorithm,Problem)
            ...
            while Algorithm.NotTerminated(OutputPop)
                ...
                %  v13: 加 objThr 守卫 -> 注入永不进 LSMOP 进化(留一手, 将来只对分子开), 保 gate。
                if bankOn && injectOn && ~isempty(objThr)
...
% 准入门两路共享: 近前沿(NDSort front-1) OR (分子)达标。准入后:
%   LSMOP(objThr=[]): 走 v12 潜 niche 准入/迁移/淘汰, 一字不动 -> == v12。
%   分子(objThr!=[]): 累积所有不同 lead(完全相同潜向量跳过), 超 BANK_CAP 时按
%                     联合(目标⊕潜)密度删最冗余(达标优先保留)。去掉潜 niche 的
%                     migrate(覆盖丢结构)/Case-C 丢弃(潜近不同结构被并)。
    ...
    if isempty(objThr)
        ...
        for i = 1:numel(Cand)
            ...
            if dmin <= r && dom(c.obj, BANK(j).obj)
                BANK(j) = c;  Bdec(j,:) = cdec;    % same niche & Pareto-better -> migrate
            elseif dmin > r
                BANK(end+1) = c;  Bdec(end+1,:) = cdec; %#ok<AGROW>  NEW direction
                if numel(BANK) > K_max
                    [BANK, Bdec] = pruneClosest(BANK, Bdec, objThr);
                end
            end
            % within r and not dominating -> discarded (== v7 on LSMOP)
        end
        return
    end

    % ===================== MOLECULAR: 累积不同 lead, 联合密度封顶 =====================
    Bdec = BANK.decs;                              % [perf] 提取一次, 循环内增量维护
    for i = 1:numel(Cand)
        cdec = Cand(i).dec;
    ...
    BANK_CAP = 300;                                % 远超所需达标点(~十几), 仅限运行时/内存
    if numel(BANK) > BANK_CAP
        BANK = jointCap(BANK, objThr, BANK_CAP);   % 超顶: 联合密度删最冗余(非达标优先, 再达标)
    end
end
...
% v13 分子: 把"所有不同的达标点"尽量送进被测输出(size N), 多样性度量用联合(目标⊕潜)空间。
%   思路: 输出 N 个槽位 >> 所需达标点(~十几), 故不激进截断 —— 把 BANK 里"输出中尚无的"
%   不同达标 lead, 注入到输出里"最冗余的非达标槽位"(联合密度最拥挤者先替换)。仅当达标 lead
%   多于可用槽位才按联合密度留最分散的一批。这同时保 (a)目标铺开 + (b)同目标不同结构。
%   LSMOP(objThr=[]) 不会走到这里(调用处已 gated), 保留 v12 dominance 分支仅作安全网。
    Out = Population;
    if isempty(BANK); return; end

    if isempty(objThr)
        % ---- 安全网: v12 dominance-only(实际不被调用, LSMOP 输出 == Population) ----
        Pobj = Out.objs;
        for i = 1:numel(BANK)
            bo  = BANK(i).obj;
            dm  = find(all(bo <= Pobj,2) & any(bo < Pobj,2));
            if ~isempty(dm)
                [~,k] = max(sum(Pobj(dm,:) - bo, 2));
                Out(dm(k)) = BANK(i);  Pobj(dm(k),:) = bo;
            end
        end
        return
    end
    ...
    % ===================== MOLECULAR: 联合密度并集选择 =====================
    Odec   = Out.decs;
    bThr   = all(BANK.objs <= objThr, 2);          % BANK 中的达标 lead
    leads  = BANK(bThr);
    if isempty(leads); return; end
    Ldec   = leads.decs;
    new    = true(numel(leads),1);                 % 输出中(按潜向量)尚无的 lead
    for i = 1:numel(leads)
        if any(all(abs(Odec - Ldec(i,:)) < 1e-12, 2)); new(i) = false; end
    end
    leads = leads(new);
    if isempty(leads); return; end

    outThr    = all(Out.objs <= objThr, 2);        % 输出里可被替换的非达标槽位
    fillSlots = find(~outThr);
    if isempty(fillSlots); return; end             % 输出已全是达标 -> 不挤掉既有达标(N足够大)
    nnOut     = jointNN(Out.objs, Out.decs);       % 槽位的联合冗余度
    [~,ord]   = sort(nnOut(fillSlots), 'ascend');  % 最冗余的非达标槽位先腾出
    nRep      = min(numel(leads), numel(fillSlots));
    slots     = fillSlots(ord(1:nRep));
    if numel(leads) > nRep                         % lead 比槽位多 -> 留最分散的 nRep 个
        Del   = jointTruncation(leads.objs, leads.decs, numel(leads) - nRep);
        leads = leads(~Del);
    end
    Out(slots) = leads(1:nRep);
end

% ---- 联合(目标⊕潜)空间多样性度量(分子专用) ----
function BANK = jointCap(BANK, objThr, CAP)
% 超容量: 按联合密度删最冗余者; 达标 lead 优先保(先删非达标, 不够再删达标)。
    n = numel(BANK);
    if n <= CAP; return; end
    prot = all(BANK.objs <= objThr, 2);
    unp  = find(~prot);
    nDrop = n - CAP;
    if numel(unp) >= nDrop
        Del = jointTruncation(BANK(unp).objs, BANK(unp).decs, nDrop);
        BANK(unp(Del)) = [];
    else
        BANK(unp) = [];                            % 先删光非达标
        nDrop2 = numel(BANK) - CAP;
        if nDrop2 > 0
            Del = jointTruncation(BANK.objs, BANK.decs, nDrop2);
            BANK(Del) = [];
        end
    end
end
...
% 块平衡联合距离: 每块 min-max 归一化后按维度数取均方(避免高维潜淹没低维目标), 开方相加。
% 同目标但潜远 -> D 大(不同结构, 留); 同目标同潜 -> D≈0(几乎同分子, 删)。
...
function nn = jointNN(Obj, Dec)
% 每行到其它行的最近联合距离(挑"最冗余槽位"用)。
    n = size(Obj,1);
    if n <= 1; nn = inf(n,1); return; end
    D = jointDist(Obj, Dec);  D(logical(eye(n))) = inf;
    nn = min(D, [], 2);
end
