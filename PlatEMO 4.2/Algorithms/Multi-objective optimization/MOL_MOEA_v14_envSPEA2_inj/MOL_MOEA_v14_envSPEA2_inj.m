classdef MOL_MOEA_v14_envSPEA2_inj < ALGORITHM
% MOL_MOEA_v14_envSPEA2_inj: == MOL_MOEA_v12_envSPEA2, 唯一改动 = **打开 L2 注入**
%   (ParameterSet injectOn 默认 0 -> 1)。其余(SPEA2环境选择/BANK/merge/全部依赖)逐字同 v12。
%
%   设计(单版本双任务, 靠 v12 自带门控自动分流):
%     L1 merge 门 = ~isempty(objThr): 分子(task=1, objThr非空)开 / LSMOP(task=0默认, objThr=[])关。
%     L2 inject 门 = injectOn(本版=1), 无 objThr 守卫 -> 两任务都注入演化。
%   => LSMOP 配置 {L0, L1=0, L2=1} (纯注入无合并, 干净看 L2 对搜索的影响, 不被 merge 的 IGD 损耗混淆);
%      分子   配置 {L0, L1=1, L2=1} (在 v12 已超 FRCSO 基础上, 看注入进演化能否再涨)。
%   注意: task 默认仍为 0 (绝不改), 否则 LSMOP 的 objThr 非空会误开 L1 门。
%
%   [以下为 v12 原说明]
    ...
    methods
        function main(Algorithm,Problem)
            ...
            [D,tau0,H,entropyBonus,learnRate,bankOn,mergeOut,injectOn,inject_max,r_frac,K_max,task] = Algorithm.ParameterSet( ...
                repmat(ceil(Problem.N.^(1/(Problem.M-1))),1,Problem.M-1), 0.7, 5, 0, 0.01, 1, 1, 1, 5, 0.1, 30, 0);  % v14: injectOn 0->1 (L2 开); task 仍=0
