...
%--------------------------------------------------------------------------
    ...
    Delta  = Upper - Lower;
    SafeDelta = Delta;
    SafeDelta(SafeDelta == 0) = 1;
    disM   = 20;
    Site   = rand(2*N,D) < 1/D;
    mu     = rand(2*N,D);
    temp   = Site & mu<=0.5;
    OffDec       = max(min(OffDec,Upper),Lower);
    OffDec(temp) = OffDec(temp)+Delta(temp).*((2.*mu(temp)+(1-2.*mu(temp)).*...
                   (1-(OffDec(temp)-Lower(temp))./SafeDelta(temp)).^(disM+1)).^(1/(disM+1))-1);
    temp  = Site & mu>0.5; 
    OffDec(temp) = OffDec(temp)+Delta(temp).*(1-(2.*(1-mu(temp))+2.*(mu(temp)-0.5).*...
                   (1-(Upper(temp)-OffDec(temp))./SafeDelta(temp)).^(disM+1)).^(1/(disM+1)));
