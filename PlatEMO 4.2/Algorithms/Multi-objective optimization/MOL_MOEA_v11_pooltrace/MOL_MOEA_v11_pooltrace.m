classdef MOL_MOEA_v11_pooltrace < ALGORITHM
% MOL_MOEA_v11_pooltrace: == MOL_MOEA_v10_bank, 仅多加 [PT] 打印诊断(搜索逻辑一字不改)。
%   每代打印三个观测点的多样性: OFF=算子产出 Offspring, SEL=EnvironmentalSelection 后(Pop_Pool 前),
%   POOL=Pop_Pool 后。每点报 N / 唯一目标点 / 唯一潜向量 / 目标范围 -> 看 spread 在哪步丢、Pop_Pool 补的是多样点还是复制。
        ...
        function main(Algorithm,Problem)
            ...
            while Algorithm.NotTerminated(OutputPop)
                ...
                fprintf('[PT] g=%d FE=%d | OFF %s | SEL %s | POOL %s\n', ...
                    genCounter, Problem.FE, pt_stat(Offspring), pt_stat(pop_sel), pt_stat(Population));
...
function s = pt_stat(P)
% [pooltrace] 多样性统计: N / 唯一目标点 / 唯一潜向量(结构近似) / obj1,obj2 范围
    if isempty(P)
        s = 'N=0'; return;
    end
    O = P.objs; Dd = P.decs;
    ...
    if size(O,2) >= 2
        s = sprintf('N=%d uObj=%d uDec=%d o1[%.3f,%.3f] o2[%.3f,%.3f]', ...
            numel(P), nuO, nuD, min(O(:,1)),max(O(:,1)),min(O(:,2)),max(O(:,2)));
    else
        s = sprintf('N=%d uObj=%d uDec=%d', numel(P), nuO, nuD);
    end
end