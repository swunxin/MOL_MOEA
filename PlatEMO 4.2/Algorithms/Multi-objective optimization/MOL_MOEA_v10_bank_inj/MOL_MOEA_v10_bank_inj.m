classdef MOL_MOEA_v10_bank_inj < ALGORITHM
% MOL_MOEA_v10_bank_inj: IDENTICAL to MOL_MOEA_v10_bank except injectOn defaults
% to 1 (L2 ON). Provided so run_parallel_experiments can run the injection
% variant by NAME only (no parameter cell). All BANK logic is unchanged.
    ...
    methods
        function main(Algorithm,Problem)
            ...
            [D,tau0,H,entropyBonus,learnRate,bankOn,mergeOut,injectOn,inject_max,r_frac,K_max,task] = Algorithm.ParameterSet( ...
                repmat(ceil(Problem.N.^(1/(Problem.M-1))),1,Problem.M-1), 0.7, 5, 0, 0.01, 1, 1, 1, 5, 0.1, 30, 0);   % injectOn=1 (L2 ON); task=0 disabled
            % task: MOMO application flag. 0 = NONE (default) -> objThr=[] -> dominance-only merge (==v7).
            %   1/2/3 = MOMO task thresholds (objectives obj = -property, "meet bar" = obj<=objThr):
            switch task
...
% Pareto-better candidate MIGRATES the rep. Deterministic (no rand) -> Stage0/1 == v7.
    ...
    FrontNo = NDSort(Cand.objs, 1);
    Cand    = Cand(FrontNo == 1);                  % objective-space gate (near-front)
    % (MOLECULAR: replace the line above with a property-threshold gate; let anchors accumulate)
    for i = 1:numel(Cand)
        c = Cand(i);
        ...
        [dmin, j] = min(d);
        if dmin <= r
            if dom(c.obj, BANK(j).obj)             % same niche & Pareto-better -> migrate
                BANK(j) = c;
            end
        else
            BANK(end+1) = c;                       % new structural direction -> add %#ok<AGROW>
            if numel(BANK) > K_max
