...
用法（服务器，激活 xin38）：
    ...
    python roundtrip_warmstart_test.py \
        --warmstart-csv /root/autodl-tmp/model_combine/MOMO-master-main/momo/data/oripops_qed/QMO_qed_mol800_optsmiles.csv \
        --leads-csv     /root/autodl-tmp/model_combine/MOMO-master-main/momo/data/qed_test.csv \
        --mol-id-range  1,200 \
        --out           outputs/roundtrip_warmstart_task1.csv
"""
import argparse
import sys
...
def main():
    ...
    ap.add_argument('--warmstart-csv', required=True, help='QMO_qed_mol800_optsmiles.csv (SMILES,mol_id,sim,qed)')
    ap.add_argument('--leads-csv', required=True, help='qed_test.csv (lead SMILES per line, 1-based by mol_id)')
