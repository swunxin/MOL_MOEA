...
def make_src_mask(src, src_pad_idx=0):
    src_mask = (src == src_pad_idx).unsqueeze(1).unsqueeze(2).to(device)  # changed for inference
    return src_mask
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask=None):
        ...
        scores = torch.matmul(Q, K.transpose(-1, -2)) / np.sqrt(
            K.shape[-1])  # scores : [batch_size, n_heads, len_q, len_k]
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input_Q, input_K, input_V, attn_mask):
        ...
        Q = self.W_Q(input_Q).view(batch_size, -1, self.num_heads, self.depth).transpose(1,
                                                                                         2)  # Q: [batch_size, n_heads, len_q, d_k]
        K = self.W_K(input_K).view(batch_size, -1, self.num_heads, self.depth).transpose(1,
                                                                                         2)  # K: [batch_size, n_heads, len_k, d_k]
        V = self.W_V(input_V).view(batch_size, -1, self.num_heads, self.depth).transpose(1,
                                                                                         2)  # V: [batch_size, n_heads, len_v(=len_k), d_v]

        # context: [batch_size, n_heads, len_q, d_v], attn: [batch_size, n_heads, len_q, len_k]
        context, attn = self.dot_product_attention(Q, K, V, attn_mask)
        context = context.transpose(1, 2).reshape(batch_size, -1,
                                                  self.d_model)  # context: [batch_size, len_q, n_heads * d_v]
...
class Encoder(nn.Module):
    def __init__(self, num_layers, d_model, num_heads, dff, input_vocab_size, maximum_position_encoding=200, rate=0.1):
        ...
        # 100 is max seq len
        self.pos_emb = PostionalEncoding(d_model, maximum_position_encoding, device=device)
    ...
    def forward(self, enc_inputs):
        ...
        #print(word_emb.shape, pos_emb.shape)
...
class EncoderForPrediction(nn.Module):
    def __init__(self, num_layers, d_model, num_heads, dff, input_vocab_size, maximum_position_encoding=200, rate=0.1,
        ...
        self.src_emb = nn.Embedding(input_vocab_size, d_model)
        self.pos_emb = PostionalEncoding(d_model, maximum_position_encoding, device=device)  # changed device for inference
        self.layers = nn.ModuleList([EncoderLayer(d_model, num_heads, dff, rate) for _ in range(num_layers)])
        self.dropout = nn.Dropout(rate)
    ...
    def forward(self, enc_inputs):
        ...
        enc_self_attn_mask = enc_self_attn_mask.repeat(1, self.num_heads, enc_self_attn_mask.shape[-1],
...
class BertModel(nn.Module):
    def __init__(self, maximum_positional_encoding, vocab_size, num_layers=6, d_model=256, dff=512, num_heads=8, dropout_rate=0.1):
        super(BertModel, self).__init__()
        self.encoder = Encoder(num_layers=num_layers, d_model=d_model,
                               num_heads=num_heads, dff=dff, input_vocab_size=vocab_size, maximum_position_encoding=maximum_positional_encoding,
                               rate=dropout_rate)
...
class PredictionModel(nn.Module):
    def __init__(self, maximum_positional_encoding, num_layers=6, d_model=256, dff=512, num_heads=8, vocab_size=60, dropout_rate=0.1, reg_nums=0,
                 clf_nums=0):
        ...
        self.encoder = EncoderForPrediction(num_layers=num_layers, d_model=d_model,
                                            ...
                                            maximum_position_encoding=maximum_positional_encoding, rate=dropout_rate,
                                            prediction_nums=self.reg_nums + self.clf_nums)

        self.fc_list = nn.ModuleList()

        for _ in range(self.clf_nums + self.reg_nums):
